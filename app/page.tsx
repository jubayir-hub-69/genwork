                    ...
                    jobs.map((job) => (
                      <div key={job.id} className="bg-[#0B1426] p-6 rounded-3xl border border-slate-800/80 shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center hover:border-slate-600 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                        <div className="mb-4 md:mb-0">
                          <span className="bg-blue-900/40 text-blue-300 text-xs font-bold px-3 py-1 rounded-full mb-3 inline-block border border-blue-800/50">Job #{job.id}</span>
                          <h3 className="text-xl font-bold text-white mb-2">{job.desc}</h3>
                          <p className="text-sm text-slate-400">
                            Status: <span className={`font-bold ml-1 ${job.status === "COMPLETED" ? "text-green-400" : job.status === "SUBMITTED" ? "text-amber-400" : "text-blue-400"}`}>{job.status}</span>
                          </p>
                        ...
                        <div className="w-full md:w-auto flex flex-col gap-3 min-w-[260px]">
                          {job.status === "OPEN" && (
                            <>
                              ...
                              <button onClick={() => handleSubmitWork(job.id)} disabled={isPending} className="bg-slate-200 text-slate-900 py-3 rounded-xl font-bold hover:bg-white hover:scale-[1.02] active:scale-95 transition-all duration-300 shadow-lg">
                                {isPending ? "Processing..." : "Submit Work"}
                              </button>
                            </>
                          )}
                          {job.status === "SUBMITTED" && (
                            <button onClick={() => handleApproveWork(job.id)} disabled={isPending} className="bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-500 hover:shadow-blue-500/20 hover:scale-[1.02] active:scale-95 transition-all duration-300 shadow-lg">
